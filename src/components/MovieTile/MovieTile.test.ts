...
// We need to set up document before importing the module since it uses `document`
let dom: JSDOM;
...
// Dynamic import to ensure document exists
async function getModule() {
  // Clear module cache so we get fresh imports with current DOM
...
describe('processFFJr', () => {
  it('should pass through normal time and title', async () => {
    const { processFFJr } = await getModule();
    const result = processFFJr('7:00', 'test film');
    expect(result.displayTime).toBe('7:00');
    expect(result.displayTitle).toBe('Test Film');
  });

  it('should strip FF Jr from time and append to title', async () => {
    const { processFFJr } = await getModule();
  ...
  it('should handle FF Jr with dash separator', async () => {
    const { processFFJr } = await getModule();
    const result = processFFJr('10:00 - FF Jr', 'test film');
    expect(result.displayTime).toBe('10:00');
    expect(result.displayTitle).toBe('Test Film (FF Jr.)');
...
describe('toTitleCase (MovieTile)', () => {
  it('should title-case basic strings', async () => {
    const { toTitleCase } = await getModule();
...
describe('createMovieElement', () => {
  it('should create element with correct base class', async () => {
    const { createMovieElement } = await getModule();
    const movie = makeMovie();
    const el = createMovieElement(movie, { isTimeline: false });
    expect(el.className).toContain('movie');
  });

  it('should add timeline class in timeline mode', async () => {
    const { createMovieElement } = await getModule();
    const movie = makeMovie();
  ...
  it('should not add timeline class in grid mode', async () => {
    const { createMovieElement } = await getModule();
    const movie = makeMovie();
    const el = createMovieElement(movie, { isTimeline: false });
  ...
  it('should add has-poster class when poster_url exists', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should not add has-poster class without poster_url', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should set timeline positioning styles', async () => {
    const { createMovieElement } = await getModule();
    const movie = makeMovie();
  ...
  it('should set only height in demo mode (no topPx)', async () => {
    const { createMovieElement } = await getModule();
    const movie = makeMovie();
  ...
  it('should render movie title in title case', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should render movie time', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should render year and director in metadata', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should limit actors to first 3', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should set data attributes for lookup', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should set tabindex and role for keyboard access', async () => {
    const { createMovieElement } = await getModule();
    const movie = makeMovie();
    const el = createMovieElement(movie, { isTimeline: false });
  ...
  it('should add overlap classes for col 1', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should add overlap-col-0 and has-overlap for col 0 with overlap', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should render single-showtime badge when highlight mode and count is 1', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should not render badge when count > 1', async () => {
    const { createMovieElement } = await getModule();
  ...
  it('should render poster img element when poster_url exists', async () => {
    const { createMovieElement } = await getModule();
    const movie = makeMovie({ poster_url: '/poster.jpg' });
    const el = createMovieElement(movie, { isTimeline: false });
